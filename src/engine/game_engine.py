...
class GameEngine:
    ...
    def _create(self):
        ...
        self.players_entity = create_world_entity(world=self.ecs_world, component_type="PLAYER", size=self.player['size'], color=self.player['color'], position=self.player['position'], velocity=self.player['velocity'])
        create_world_entity(world=self.ecs_world, component_type="INPUT_COMMAND", name="PLAYER_LEFT", key=pygame.K_LEFT)
        create_world_entity(world=self.ecs_world, component_type="INPUT_COMMAND", name="PLAYER_RIGHT", key=pygame.K_RIGHT)
        ...
        create_world_entity(world=self.ecs_world, component_type="INPUT_COMMAND", name="PLAYER_FIRE", key=pygame.BUTTON_LEFT)
    ...
    def _draw(self):
        self.screen.fill((self.background_color.get('r'), self.background_color.get('g'), self.background_color.get('b')))
    ...
    def _do_action(self, c_input: CInputCommand):
       player_component = self.ecs_world.component_for_entity(entity=self.players_entity, component_type=CPlayerTag)
